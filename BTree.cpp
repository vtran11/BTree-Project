...
void BTree::print()
{
  ...
  while(!queue.isEmpty())
  {
    ...
    BTreeNodePtr->print(queue); //print the root then the root after deleted
  } // while
} // BTree::print()
...
{  // To be written by students
    BTreeNode *ptr1 = root-> remove(value);
    if(ptr1)  //if root split
    {
      delete root; //point ptr1 to remove function and then delete it
        root = ptr1;
        root->setParent(NULL); //no merge
    }
}